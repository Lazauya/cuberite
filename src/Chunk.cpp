...
void cChunk::TickBlocks(void)
{
	...
	for (int i = 0; i < 50; ++i)
	{
		...
		idx = (idx + 1 + m_World->GetTickRandomNumber(cChunkDef::NumBlocks - 2)) % cChunkDef::NumBlocks;
	...
	m_BlockToTick = cChunkDef::IndexToCoordinate(static_cast<size_t>(m_World->GetTickRandomNumber(cChunkDef::NumBlocks - 1)));
}
